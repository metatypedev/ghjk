    ...
    logDepthObj,
    {
      ...
      logWarn(firstArg: string, ...data: any[]) {
        logStep(
          firstArg,
          data,
          (t) => colors.bold(colors.yellow(t)),
          state.warnLogger.getValue(),
        );
      },
      logGroup<TResult>(
        labelOrAction?: string | (() => TResult),
        maybeAction?: () => TResult,
      ): TResult | void {
        const label = typeof labelOrAction === "string"
          ? labelOrAction
          : undefined;
        if (label) {
        ...
        const action = label != null
          ? maybeAction
          : labelOrAction as (() => TResult);
        if (action != null) {
      ...
      commandExists(commandName: string) {
        if (
          state.commandBuilder.getValue()[getRegisteredCommandNamesSymbol]()
      ...
      prompt,
      progress(
        messageOrText: ProgressOptions | string,
        options?: Omit<ProgressOptions, "message" | "prefix">,
      ) {
      ...
      setPrintCommand(value: boolean) {
        const commandBuilder = state.commandBuilder.getValue().printCommand(
          value,
        );
        state.commandBuilder.setValue(commandBuilder);
      },
